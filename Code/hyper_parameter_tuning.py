...
#%%
x_train,x_test, y_train, y_test= train_test_split(feat_data2.drop('winner_le',axis=1),feat_data2['winner_le'],test_size=0.2, random_state=5)
...
# history= nn_model.fit(x_train, y_train, epochs=2, batch_size=8,verbose=0,validation_data= (x_test,y_test))
...
history= ho_model.fit(x_train,y_train,epochs=700,batch_size=best_param_nn['batch_size'],callbacks=[callback],verbose =1,validation_data=(x_test,y_test))
...
trials_xgb = Trials()
best_xgb = fmin(f_xgb, space_xgb, algo=tpe.suggest, max_evals=500,trials=trials_xgb)
